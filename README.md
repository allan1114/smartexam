...
- **可調整的設置**：
  ...
  - 考試時長（30-240 分鐘）
  - 問題順序（順序/隨機）
  - 內容範圍焦點 Focus Range：題號範圍（如 `Question 179-250`）、章節，或**頁數範圍**（如 `Pages 1-20`）
    - 用 **MiniMax** 讀長 PDF 時，頁數範圍會**實際只讀該幾頁**（MiniMax 要逐頁轉圖，長文件必須分段；每段有獨立題庫）
    - 用 **Google** 則毋須分段——Gemini 可一次讀完整份 PDF
    - 填入題號範圍時，AI 會跳過範圍以外嘅題目，並為該範圍單獨建立題庫（唔會與整份文件嘅題庫混淆）
...
- **📄 原文題庫模式（上傳嘅 PDF 就係唯一題源）**：考試設定最頂有「**題目來源**」二選一,揀 **📄 原文抽取** 之後,上傳嘅文件就係唯一參考,系統只會**逐字複製**入面已有嘅題目同答案:
  - **絕不自創題目**。AI 收到嘅指令入面根本冇「生成題目」呢個分支——冇分支可以行錯落去
  - **一個字都唔改**：題目、選項、正確答案全部 CHARACTER-FOR-CHARACTER 照抄,唔會改寫、翻譯、簡化或者「執靚」
  - **temperature 鎖死 0**,即使你喺設定頁調高咗都會被忽略（抄寫只有一個正確答案）
  - **全部題目、原本次序**：唔會抽樣、唔會被題數上限截住、唔會打亂題目或選項次序（即使揀咗 Randomized 亦會被原文模式覆蓋）
  - **抽唔到就報錯,唔會靜靜地出 AI 題**：若文件本身冇現成題目,會直接顯示 `NO_QUESTIONS_IN_DOCUMENT` 並叫你改用「✨ AI 生成」
  - 註：每題嘅**解釋**仍然由 AI 撰寫（以文件原文出處為依據）——係附加嘅溫習輔助,唔屬於原文改動
- **✨ AI 生成模式**：文件係筆記／課本／簡報等學習材料時揀呢個,行為同以前一樣（可自選題數、次序、剔選 **Use every question** 載入全部）
  - 題庫面板會顯示抽取狀態：✅ 已完整抽取,或 ⚠️ 未確認完整（建議按 Regenerate 重試）
  - 若抽取未完成,開始考試時亦會有黃色提示,唔會靜靜地當作全份載入
  - 若快取到嘅題庫係 AI 生成（CASE B）而你揀咗原文模式,面板會紅字提示,開考時會重新以原文模式抽取
- **🛡️ 更穩健嘅考試生成**:
